...
## boxty.billing.BillingReportItem
...
## boxty.billing.WorkspaceBillingReportItem
...
## boxty.billing.workspace_billing_report
...
The result will be a list of dictionaries for each interval (determined by resolution) between the start and end limits. The dictionary represents a single Boxty object that billing can be attributed to (e.g., an App) along with metadata (including user-defined tags) for identifying that object. The dictionary also contains a breakdown of the cost value attributed to individual resources (for an App, this can be CPU, Memory, specific GPU types, etc.). The specific resource types included in the breakdown are subject to change as Boxty's billing model evolves.
...
It's also possible to generate reports using the boxty billing report CLI command. The CLI has a few convenience features for generating reports across relative time ranges.
