...
Expose a port publicly from inside a running Boxty container, with TLS.
...
app = boxty.App(image=boxty.Image.debian_slim().pip_install("Flask"))
...
@app.function()
def run_app():
    # Start a web server inside the container at port 8000. `boxty.forward(8000)` lets us
    # expose that port to the world at a random HTTPS URL.
    with boxty.forward(8000) as tunnel:
...
def tcp_tunnel():
    ...
    with boxty.forward(8000, unencrypted=True) as tunnel:
...
SSH example: This assumes you have a rsa keypair in ~/.ssh/id_rsa{.pub}, this is a bare-bones example letting you SSH into a Boxty container.
...
def some_function():
    ...
    with boxty.forward(port=22, unencrypted=True) as tunnel:
