...
```python
class Volume(boxty.object.Object)
```

A writeable volume that can be used to share files between one or more Boxty functions.
...
app = boxty.App()
volume = boxty.Volume.from_name("my-persisted-volume", create_if_missing=True)
...
This does not return a local handle; use boxty.Volume.from_name to look up the Volume after creation.

Added in v1.1.2.

### Parameters

- `name` (str): Name for the new Volume.
- `version` (int | None): Optional VolumeFS backend version (1 or 2); experimental.
- `allow_existing` (bool): If True, do nothing when a Volume with this name already exists. (Default is False)
...
```python
boxty.Volume.objects.create("my-volume")
```

Volumes will be created in the active environment, or another one can be specified:

```python
boxty.Volume.objects.create("my-volume", environment_name="dev")
```

By default, an error is raised if the Volume already exists; allow_existing=True makes that case a no-op:

```python
boxty.Volume.objects.create("my-volume", allow_existing=True)
```

Note that this method does not return a local instance of the Volume. You can use boxty.Volume.from_name to perform a lookup after creation.
...
- `max_objects` (int | None): Maximum number of Volumes to return.
- `created_before` (datetime | str | None): Only include Volumes created before this time (datetime or ISO date string).
...
```python
volumes = boxty.Volume.objects.list()
...
```python
dev_volumes = boxty.Volume.objects.list(environment_name="dev")
```
...
```python
volumes = boxty.Volume.objects.list(max_objects=10, created_before="2025-01-01")
```
...
- `allow_missing` (bool): If True, do nothing when the Volume does not exist. (Default is False)
...
```python
await boxty.Volume.objects.delete("my-volume")
```

Volumes will be deleted from the active environment, or another one can be specified:

```python
await boxty.Volume.objects.delete("my-volume", environment_name="dev")
```
...
Note that these options are not properties stored with the Volume itself - they can be individually configured for each Volume - container association.

### Parameters

- `read_only` (bool | None): Set this to True to make the Volume read only from within containers.
- `sub_path` (str | PurePosixPath | None): Only mount this sub_path directory from the Volume. If the directory doesn't exist in the Volume, it will be created when the container starts up.
...
volume = boxty.Volume.from_name("my-volume")
...
- `name` (str): Deployment name of the Volume.
- `environment_name` (str | None): Environment to resolve the name in; defaults to the active environment.
- `create_if_missing` (bool): If True, create the Volume when it does not already exist. (Default is False)
- `version` ("boxty_proto.api_pb2.VolumeFsVersion.ValueType | None"): Optional VolumeFS backend version; must match an existing Volume when set.
...
```python
vol = boxty.Volume.from_name("my-volume", create_if_missing=True)
...
- `volume_id` (str): Volume object ID to attach to.
- `client` (_Client | None): Boxty client to use for loading; defaults to Client.from_env() when omitted.
...
def my_worker(volume_id: str):
    vol = boxty.Volume.from_id(volume_id)
    for entry in vol.listdir("/"):
        print(entry.path)

with boxty.Volume.ephemeral() as vol:
...
- `client` (_Client | None): Boxty client to use; defaults to Client.from_env() when omitted.
- `environment_name` (str | None): Environment for the ephemeral Volume; defaults to the active environment.
- `version` ("boxty_proto.api_pb2.VolumeFsVersion.ValueType | None"): Optional VolumeFS backend version for the ephemeral Volume.
...
```python
async with boxty.Volume.ephemeral() as vol:
...
List all files under a path prefix in the boxty.Volume.
...
Read a file from the boxty.Volume.

Note - this function is primarily intended to be used outside of a Boxty App. For more information on downloading files from a Boxty Volume, see the guide.

### Parameters

- `path` (str): Path to the file inside the Volume.

### Usage

```python
vol = boxty.Volume.from_name("my-boxty-volume")
data = b""
...
Note that if the volume is already mounted on the Boxty function, you should use normal filesystem operations like os.rename() and then commit() the volume. The copy_files() method is useful when you don't have the volume mounted as a filesystem, e.g. when running a script on your local computer.

### Parameters

- `src_paths` (Sequence[str]): Source paths inside the Volume (list of one or more paths).
- `dst_path` (str): Destination path inside the Volume (file or directory, following cp semantics).
- `recursive` (bool): Whether to copy directories recursively (V2 volumes only). (Default is False)
...
- `force` (bool): If True, allow overwriting existing files with uploads (not directories). (Default is False)
