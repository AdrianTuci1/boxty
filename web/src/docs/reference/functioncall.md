...
```python
class FunctionCall(typing.Generic, boxty.object.Object)
```

A reference to an executed function call.

Constructed using `.spawn(...)` on a Boxty function with the same arguments that a function normally takes. Acts as a reference to an ongoing function call that can be passed around and used to poll or fetch function results at some later time.
...
`.spawn()` calls have a single output, so only specifying index=0 is valid. A non-zero index is useful when your function has multiple outputs, like via `.spawn_map()`.
...
- `timeout` (float | None): Maximum seconds to wait for a result, or None to wait indefinitely.
- `index` (int): Which input's result to retrieve (typically 0 for `.spawn()`). (Default is 0)
...
- `start` (int): First input index to include (inclusive). (Default is 0)
- `end` (int | None): One past the last index to include, or None for all remaining inputs.
...
See [boxty.call_graph](call_graph.md) reference page for documentation on the structure of the returned InputInfo items.
...
- `terminate_containers` (bool): If True, forcibly terminate workers running cancelled inputs. (Default is False)
...
- `function_call_id` (str): Object ID of an existing function call (e.g. from FunctionCall.object_id).
...
Wait until all Boxty FunctionCall objects have results before returning.

Accepts a variable number of FunctionCall objects, as returned by Function.spawn().

Raises an exception from the first failing function call.

Added in v0.73.69: This method replaces the deprecated boxty.functions.gather function.

### Parameters

- `*function_calls` ("_FunctionCall[T]"): FunctionCall instances to wait on (same order as the returned sequence).
...
result_1, result_2 = boxty.FunctionCall.gather(fc1, fc2)
```