      ...
      <table className="min-w-full text-left text-xs">
        <thead>
          <tr className="bg-[#111111] border-b border-[#262626]">
            ...
            <th className="px-4 py-2.5 text-gray-500 font-medium">Started</th>
            <th className="px-4 py-2.5 text-gray-500 font-medium">CPU</th>
        ...
        <tbody className="divide-y divide-[#262626]">
          {sandboxes.map((s) => (
            <tr key={s.id} className="bg-[#161616] hover:bg-[#1e1e1e] transition-colors cursor-default">
              ...
              <td className="px-4 py-3 text-gray-500">{s.started_at ? new Date(s.started_at).toLocaleString() : '-'}</td>
              <td className="px-4 py-3 text-gray-300">{s.cpu_avg_pct ?? '-'}%</td>
              <td className="px-4 py-3 text-gray-300">{s.memory_avg_mb ?? '-'} MB</td>
